...
const Shader* ShaderCache::tryLoadShader(const ShaderKey& shaderKey) {
    ...
    if (const auto cachedVertexSource = getSource(vertexSourceCache, shaderKey.vertexPath)) {
        ...
        if (!vertexSourceFs) {
            log::error("failed to read vertex shader at path {}: {}", vertexPath, vertexSourceFs.unwrapErr());
            return nullptr;
        }
    ...
    if (const auto cachedFragmentSource = getSource(fragmentSourceCache, shaderKey.fragmentPath)) {
        ...
        if (!fragmentSourceFs) {
            log::error("failed to read fragment shader at path {}: {}", fragmentPath, fragmentSourceFs.unwrapErr());
            return nullptr;
        }
